...
- 헐 셰이더는 패치당 한 번 호출되어, 낮은 차수의 표면을 정의하는 입력 제어점을
  패치를 구성하는 제어점으로 변환

04. TS (분할기, Tessellator)
- 테셀레이션 단계는 하드웨어에서 테셀레이션을 구현
- 그래픽 파이프라인은 낮은 디테일 (낮은 다각형 수) 모델을 평가하고 더 높은 디테일로
  렌더링 할 수 있다
- 더 높은 차수의 표면을 Direct3D 그래픽 파이프라인 내에서 자세한 렌더링을 위한 많은
  삼각형으로 변하는데에 함께 작동
...
- 도메인 쉐이더에서 생성된 도형 정보를 가지고 추가적인 변환을 수행하거나,
  기하학적 모양을 만들어 낸다

07. SO (스트림 출력, Stream Output)
- 정점 데이터를 캡처하고 저장

08. RS (레스터라이저, Rasterizer)
- 3D 공간에서 정의된 3D 모델을 2D 화면으로 변환하는 과정
- 각 3D 폴리곤을 2D 픽셀로 변환하고, 이 픽셀에 대한 정보(색상, 깊이 등)을 계산하여
  RTV에 출력

09. PS (픽셀 쉐이더, Pixel Shader)
- 화면에 출력되는 각 필셀에 대한 정보(색상, 깊이 등)를 계산
